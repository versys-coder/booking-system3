  ...
  <div className="poolworkload-popup">
    ...
    {!isBreak ? (
      <>
        ...
        {onBook && (
          <button
            className="poolworkload-book-btn"
            style={{
              marginTop: 9,
              width: "100%",
              padding: "8px 0",
...
interface PoolWorkload2Props {
  onBook?: (date: string, hour: number) => void;
}

const PoolWorkload2: React.FC<PoolWorkload2Props> = ({ onBook }) => {
  ...
  // === POPUP LOGIC ===
  function showPopupAtClient(clientX: number, clientY: number, val: number, h: number, br: boolean, date: string) {
  ...
  const makeMouseHandlers = (h: number, val: number, br: boolean, date: string) => ({
    onMouseEnter: (e: React.MouseEvent) => showPopupAtClient(e.clientX, e.clientY, val, h, br, date),
    onMouseMove: (e: React.MouseEvent) => {
      cancelHide();
      showPopupAtClient(e.clientX, e.clientY, val, h, br, date);
    },
    ...
    onTouchStart: (e: React.TouchEvent) => {
      ...
      if (!t) return;
      cancelHide();
      showPopupAtClient(t.clientX, t.clientY, val, h, br, date);
    },
          ...
          >
            ...
            <div className="poolworkload-chart-overlay">
              {hourColumns.map((h, hourIdx) => {
                ...
                const handlers = makeMouseHandlers(h, val, br, date);
                return (
                  <div
                    key={`overlay-${h}`}
            ...
            {popup && containerRef.current && (
              ...
              >
                <ColumnPopup
                  value={popup.value}
                  hour={popup.hour}
                  isBreak={popup.isBreak}
                  onBook={
                    !popup.isBreak && onBook
                      ? () => onBook(popup.date, popup.hour)
                      : undefined
                  }
                />
              </div>
