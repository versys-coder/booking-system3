...
// Константы как в оригинальном PoolWorkload1
const BAR_WIDTH = 50;
const BAR_GAP = 5;
...
const BAR_TOTAL_HEIGHT =
  TOTAL_LANES * SEGMENT_HEIGHT + (TOTAL_LANES - 1) * SEGMENT_GAP;
...
const PoolWorkload1: React.FC = () => {
  ...
  const [selectedIdx, setSelectedIdx] = useState(0); // активная дата
  ...
  // важное: ширина области под все часы
  const chartWidth = useMemo(() => {
    // LEFT_PADDING слева под метку даты, потом все колонки часов
    return LEFT_PADDING + hours.length * (BAR_WIDTH + BAR_GAP) - BAR_GAP;
  }, [hours]);
    ...
    <div className="poolworkload-root">
      <div className="poolworkload-card">
          ...
          {dates.map((d, idx) => {
            ...
            return (
              <div
                key={d}
                className={
                  "poolworkload-date" + (active ? " poolworkload-date-active" : "")
                }
              ...
              >
                ...
                <div className="poolworkload-date-labels">
                  <span className="poolworkload-date-main">
                    {formatDateShortRu(d)}
                  </span>
                  <span className="poolworkload-date-weekday">
                    {getWeekdayRu(d)}
                  </span>
                </div>
              </div>
        ...
        <div ref={scrollRef} className="poolworkload-chart-scroll">
          ...
          >
            {/* Шапка часов: в одну строку, без переноса */}
            <div
            ...
            {activeDate && (
              <div className="poolworkload-row" style={{ display: "flex", alignItems: "flex-end" }}>
                {/* Метка даты слева */}
                <div
