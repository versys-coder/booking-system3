...
function readEmbedConfigFromUrl() {
  try {
    ...
    const mode: "minimal" | "compact" =
      modeParam === "compact" ? "compact" : "minimal";
    const transparentBg =
      bg === "transparent" || bg === "none" || sp.get("noBg") === "1";
...
function short(s?: string | null, max = 200) {
  if (!s) return "";
...
function installFetchLogger() {
  ...
  (window as any).__PW_FETCH_LOG_PUSH__ = (entry: FetchLogEntry) => {
    (window as any).__PW_FETCH_LOG_QUEUE__ =
      (window as any).__PW_FETCH_LOG_QUEUE__ || [];
    (window as any).__PW_FETCH_LOG_QUEUE__.push(entry);
  };
    ...
    try {
      ...
      if (initHeaders) {
        if (initHeaders instanceof Headers)
          ...
          (initHeaders as any[]).forEach(([k, v]) => (headersObj[k] = v));
    ...
    try {
      ...
      try {
        resp.headers.forEach((v, k) => {
          (entryBase.responseHeaders!)[k] = v;
        });
      } catch (e) {}
      try {
        entryBase.responseText = await resp.clone().text();
      } catch (e) {
        entryBase.responseText = `<<unable to read body>>`;
      }
      entryBase.durationMs = ms;
      try {
        (window as any).__PW_FETCH_LOG_PUSH__ &&
          (window as any).__PW_FETCH_LOG_PUSH__(entryBase);
      } catch (e) {}
      return resp;
    } catch (err: any) {
      const ms = Date.now() - tsStart;
...
function DebugPanel({
  ...
  const [logs, setLogs] = useState<FetchLogEntry[]>(() => {
    const q = (window as any).__PW_FETCH_LOG_QUEUE__ || [];
    (window as any).__PW_FETCH_LOG_QUEUE__ = [];
    return Array.isArray(q) ? q.slice().reverse() : [];
  });
  ...
  useEffect(() => {
    ...
    (window as any).__PW_FETCH_LOG_PUSH__ = (entry: FetchLogEntry) => {
      try {
        pushRef.current && pushRef.current(entry);
      } catch {}
    };
    return () => {
      (window as any).__PW_FETCH_LOG_PUSH__ = (entry: FetchLogEntry) => {
  ...
  async function runTestConfirm() {
    try {
      const phone = prompt("Phone to test:", appState.phone || "");
      if (!phone) return;
      const base =
      ...
      alert(`Response ${resp.status}: ${short(text, 2000)}`);
    } catch (e: any) {
      alert("Test failed: " + (e?.message || String(e)));
    }
  }
      ...
      <div style={{ display: "flex", height: "calc(100% - 44px)" }}>
        ...
        >
                ...
                onClick={() => {
                  try {
                    alert(
                      JSON.stringify(
                        Object.fromEntries(
                          Object.entries(localStorage as any)
                        ),
                        null,
                        2
                      )
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#9fb3d6" }}>
              - Window messages (postMessage & custom events)
            </div>
                ...
                onClick={() => {
                  try {
                    window.parent.postMessage?.(
                      {
                        type: "dvvs:debug:dump",
                        payload: getAppState()
                      },
                      "*"
                    );
                    alert("Posted dvvs:debug:dump to parent");
                  } catch (e) {
          ...
          >
            ...
            >
              <input
                type="checkbox"
                checked={showResp}
                onChange={(e) => setShowResp(e.target.checked)}
              />{" "}
              show responses
            </label>
          </div>
            ...
            {logs
              ...
              .map((l) => (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 700,
                        color:
                          l.status && l.status >= 200 && l.status < 300
                            ? "#4ade80"
                            : "#f87171"
                      }}
                    >
                  ...
                  >
                    <div>
                      ts: {l.ts} · duration: {l.durationMs ?? "-"}ms
                    </div>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      {short(
                        l.requestBody ??
                          JSON.stringify(l.requestHeaders || {}),
                        2000
                      )}
                    </pre>
                  </div>
...
type PaymentInfo = { bookingId: string; amount: number };

function calcTestAmount(dateIso: string, hour: number): number {
  const day = new Date(dateIso).getDay(); // 0-вс, 1-пн,...,6-сб
  const isWeekday = day >= 1 && day <= 5;
  // тестовая логика: вместо 600/650 руб — 1/2 руб
  if (isWeekday && hour >= 7 && hour < 16) return 1;
  return 2;
}
...
const WizardApp: React.FC = () => {
  ...
  const [debugVisible, setDebugVisible] = useState<boolean>(
    Boolean(new URLSearchParams(window.location.search).get("pw_debug"))
  );

  useEffect(() => {
    try {
      const p = localStorage.getItem("pw:phone");
      if (p) setPhone(p);
      const prevStart = localStorage.getItem("pw:selectedStart");
      const prevAppt = localStorage.getItem("pw:selectedAppointmentId");
      if (prevStart) {
        const dt = new Date(prevStart);
        if (!isNaN(dt.getTime())) {
          setSelectedDate(dt.toISOString().slice(0, 10));
          setSelectedHour(dt.getHours());
          setSelectedSlot({
            start_date: prevStart,
            appointment_id: prevAppt || undefined
          });
        }
      }
    } catch {}
      ...
      if (payload.start) {
        ...
        if (!isNaN(dt.getTime())) {
          ...
          const appt =
            ...
            localStorage.getItem("pw:selectedAppointmentId") ||
            undefined;
          setSelectedDate(dt.toISOString().slice(0, 10));
          setSelectedHour(dt.getHours());
          setSelectedSlot({ start_date: startIso, appointment_id: appt });
          setStep(3);
          return;
        }
      }
      ...
      try {
        ...
        if (start) {
          ...
          if (!isNaN(dt.getTime())) {
            setSelectedDate(dt.toISOString().slice(0, 10));
            setSelectedHour(dt.getHours());
            setSelectedSlot({ start_date: start, appointment_id: appt });
            setStep(3);
            return;
          }
        }
  ...
  const handleSlotSelected = useCallback((dateIso: string, hour: number) => {
    let appt: string | undefined = undefined;
    try {
      appt = localStorage.getItem("pw:selectedAppointmentId") || undefined;
    } catch {}
    ...
    setSelectedSlot({ start_date: start, appointment_id: appt });
    try {
      localStorage.setItem("pw:selectedStart", start);
    } catch {}
    setStep(3);
  }, []);
  ...
  function getSelectedSlot() {
    if (selectedSlot && (selectedSlot.appointment_id || selectedSlot.start_date))
      return selectedSlot;
    if (selectedDate && selectedHour != null) {
      const start = `${selectedDate}T${String(selectedHour).padStart(
        2,
        "0"
      )}:00:00`;
      const appt = (() => {
        try {
          return localStorage.getItem("pw:selectedAppointmentId") || undefined;
        } catch {
          return undefined;
        }
      })();
      return { start_date: start, appointment_id: appt };
    }
    return null;
  }

  function onNewBookingReset() {
    setStep(1);
    setClient(null);
    ...
    setPaymentInfo(null);
    try {
      localStorage.removeItem("pw:phone");
      localStorage.removeItem("pw:requestId");
      localStorage.removeItem("pw:selectedStart");
      localStorage.removeItem("pw:selectedAppointmentId");
    } catch {}
  }
  ...
  const getAppState = useCallback(
    () => ({
      ...
      PW_API_BASE: (window as any).__PW_API_BASE__ || null,
      href: window.location.href,
      userAgent: navigator.userAgent
    }),
    [step, selectedDate, selectedHour, selectedSlot, phone, requestId]
  );
      ...
      <div className="wiz-container" style={containerStyle}>
        ...
        {step === 3 && (
          <PhoneForm
            ...
            initialPhone={phone}
            onBack={() => setStep(2)}
            onSubmit={({ phone: p, requestId: rid }) => {
              setPhone(p);
              setRequestId(rid);
              setStep(4);
            }}
        ...
        {step === 4 && (
          <SmsForm
            ...
            confirmDelayMs={800}
            onComplete={(res) => {
              const booking = res?.bookingResult ?? res;
              setClient(booking);
              ...
              if (slot?.start_date) {
                dateIso = slot.start_date.slice(0, 10);
                hour = Number(slot.start_date.slice(11, 13)) || null;
              } else if (selectedDate && selectedHour != null) {
                dateIso = selectedDate;
                hour = selectedHour;
              }

              let amount = 1;
              if (dateIso && hour != null) {
                amount = calcTestAmount(dateIso, hour);
              }

              const bookingId =
                booking?.data?.appointment?.id ||
                booking?.appointment?.id ||
                slot?.appointment_id ||
                "UNKNOWN";

              setPaymentInfo({ bookingId, amount });
        ...
        {step === 5 && (
          <>
            <Success client={client} onNewBooking={onNewBookingReset} />
            {paymentInfo && (
              <div
                style={{
                  marginTop: 16,
              ...
              >
                ...
                <div style={{ marginTop: 6, fontSize: 12, opacity: 0.7 }}>
                  Оплата необязательна и не отменяет вашу запись.
                </div>
              </div>
            )}
          </>
        ...
        {step === "payment" && paymentInfo && (
          <Payment
            ...
            qrPath={qrcodeSvg}
            onPaid={(res) => {
              console.log("onPaid from WizardApp", res);
      ...
      <div>
        <button
          ...
          style={{
            ...
            background: debugVisible ? "#ef4444" : "#111827",
            color: "#fff",
            cursor: "pointer",
